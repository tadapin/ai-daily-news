...
description: 43本のRSSフィードから過去24時間のAIニュースを収集・要約・重複排除・AI関連フィルタリングして、このリポジトリの`docs/`に日次HTMLを生成するCodex用スキル。「Daily AI Newsを更新して」「今日のAIニュースを公開して」などの依頼や、Codexオートメーションでの日次更新時に使用する。
---

# Daily AI News Generator

43本のRSSフィードから過去24時間のAIニュースを収集し、日本語サマリー付きHTMLを生成して、このリポジトリのGitHub Pages用`docs/`を更新する。
...
- リポジトリルートの`.env`に `GEMINI_API_KEY` を設定しておく
...
`fetch_daily.py` の処理：
1. 43フィードから過去24時間の記事を取得
2. タイトルベースの重複排除（SequenceMatcher、閾値0.75）
3. Gemini 3 Flash で日本語サマリー生成（2〜3文、十分な長さ）
4. Gemini 3 Flash で意味的重複排除（同一トピックの複数報道を1件に統合）
5. Gemini 3 Flash で AI 関連フィルタリング（AI/ML/LLM/自動化に無関係な記事を除去）
6. `daily-ai-news-generator/output/daily_articles.json` に出力
